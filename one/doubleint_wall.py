...
class DoubleIntWall(Task):
    ...
    def h_components(self, state: State) -> HFloat:
        self.chk_x(state)
        p, v = state
        # keeps the pos withing the [-1.0, 1.0] boundary
        h_p_ub = p - 1.0 #  ensures the pos is less than or equal to 1.0 --> safe when p <= 1.0
        h_p_lb = -(p + 1.0) # ensures the pos is greater than or equal to -1.0 --> safe when p >= -1.0
